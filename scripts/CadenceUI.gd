...
## Cadence UI — Timer, accuracy counter, reward preview, game-over / win
## overlays, and settings panel.  Mirrors ConjurersChorusUI / IceFishingUI
## structure for consistency across minigames.
...
func _ready():
	...
	# ── Accuracy label (top-right, below timer) ──
	_accuracy_label = Label.new()
	_accuracy_label.name = "AccuracyLabel"
	if _pixel_font:
		_accuracy_label.add_theme_font_override("font", _pixel_font)
	_accuracy_label.add_theme_font_size_override("font_size", 26)
	_accuracy_label.add_theme_color_override("font_color", Color(0.85, 0.82, 0.7, 1.0))
	_accuracy_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_RIGHT
	_accuracy_label.set_anchors_preset(Control.PRESET_TOP_RIGHT)
	_accuracy_label.offset_left = -200
	_accuracy_label.offset_top = 96
	_accuracy_label.offset_right = -8
	_accuracy_label.offset_bottom = 126
	...
	# ── Big title label (win/lose splash) ──
...
# ── Called by CadenceGame when the song actually starts ──
...
func update_accuracy_display(hits: int, total: int) -> void:
	...
	if pct >= 75:
		...
		_accuracy_label.add_theme_color_override("font_color", Color(1.0, 0.9, 0.3, 1.0))
	else:
		_accuracy_label.add_theme_color_override("font_color", Color(1.0, 0.45, 0.35, 1.0))
...
# ── Game over (single-performance loss — always final) ──
...
func show_game_over(is_final: bool):
	...
	if is_final:
		...
		if lose_music:
			lose_music.play()
		if _big_title_label:
			_big_title_label.text = "Off Beat!"
...
func show_win():
	...
	if _big_title_label:
		_big_title_label.text = "Perfect Rhythm!"
