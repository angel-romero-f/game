...
## Conjurer's Chorus UI — Timer, reward preview, game-over / win overlays, settings.
## Mirrors CourtlyCuisineUI / BridgeGameUI structure for consistency across minigames.
...
# ── Game over (wrong note — always final, no retry) ──
...
func show_game_over(is_final: bool):
	...
	if is_final:
		if _big_title_label:
			_big_title_label.text = "The Choir Falls Silent!"
...
func show_win():
	...
	if _big_title_label:
		_big_title_label.text = "The Chorus Triumphs!"
